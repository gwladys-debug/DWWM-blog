        ...
        <div class="flex items-center justify-between border-b border-slate-800 pb-5">
            <div>
                <h1 class="text-2xl font-bold font-mono text-slate-100 tracking-tight flex items-center gap-3">
                    <span class="text-cyan-400">~/admin/articles/</span>{{ isset($article) ? 'modifier' : 'nouveau' }}
                </h1>
                <p class="text-xs font-mono text-slate-400 mt-1">
                    // {{ isset($article) ? 'Modifier l\'article existant' : 'Rédiger et publier un nouvel article' }}
                </p>
            </div>
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-xl p-6 backdrop-blur-xl shadow-2xl space-y-6">
                ...
                <div>
                    ...
                    <textarea name="excerpt" id="excerpt" rows="3" required
                        ...
                        placeholder="Un court résumé de 2 à 3 phrases qui apparaîtra sur les cartes d'articles...">{{ old('excerpt', $article->excerpt ?? '') }}</textarea>
                    @error('excerpt')
                        <p class="text-rose-400 text-xs font-mono mt-1">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" style="background-color: #020617; color: #f8fafc;"
                            ...
                            <option value="PUBLISHED"
                                {{ old('status', $article->status ?? '') == 'PUBLISHED' ? 'selected' : '' }}>Publié
                            </option>
                            <option value="DRAFT"
                                {{ old('status', $article->status ?? '') == 'DRAFT' ? 'selected' : '' }}>Brouillon</option>
                        </select>
                    </div>
                </div>

                {{-- 🏷️ SECTION TAGS (Ajoutée) --}}
                <div>
                    <label class="block text-xs font-mono text-slate-300 font-semibold mb-2">
                        <span class="text-cyan-400">#</span> Tags associés
                    </label>
                    <div class="flex flex-wrap gap-3 p-4 bg-slate-950 border border-slate-800 rounded-lg">
                        @forelse($tags as $tag)
                            <label
                                class="inline-flex items-center gap-2 cursor-pointer text-xs font-mono text-slate-300 hover:text-cyan-400 transition-colors">
                                <input type="checkbox" name="tags[]" value="{{ $tag->id }}"
                                    @checked(in_array($tag->id, old('tags', isset($article) ? $article->tags->pluck('id')->toArray() : [])))
                                    class="rounded bg-slate-900 border-slate-700 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-slate-950">
                                #{{ $tag->name }}
                            </label>
                        @empty
                            <p class="text-xs font-mono text-slate-500">// Aucun tag disponible en BDD</p>
                        @endforelse
                    </div>
                    @error('tags')
                        <p class="text-rose-400 text-xs font-mono mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="12" required
                        ...
                        placeholder="Écrivez votre article ici... Utilisation des titres ### et du gras **autorisée**.">{{ old('content', $article->content ?? '') }}</textarea>
                    @error('content')
                        <p class="text-rose-400 text-xs font-mono mt-1">{{ $message }}</p>
                    @enderror
                </div>
